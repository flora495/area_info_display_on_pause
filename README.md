...
1. **Enabled** - mod全体の有効/無効
2. **Show Total** - 総数表示のパターン
3. **Attempt Counter** - 挑戦回数表示のON/OFF
4. **Personal Best** - 最高到達地点表示のON/OFF
5. **Progression Detail** - 通常表示の代わりにエリアごとの詳細記録を表示するON/OFF（ONの間は3・4の設定は意味を持たないためグレーアウトされます）
...
| `Settings.cs` | 有効/無効・総数表示パターン・挑戦回数カウンター・Personal BestのON/OFFを保持する設定データ。XMLでの読み込み/保存 |
| `EnabledToggle.cs` / `AttemptCounterToggle.cs` / `PersonalBestToggle.cs` / `ProgressionDetailToggle.cs` | メイン・ポーズ両メニュー上のON/OFFチェックボックス |
