...
- 3つの「Babe」エンディング画面にちょうどいる間は、通常のエリア表示・PB表示の代わりに`Babe 1`/`Babe 2`/`Babe 3`と表示されます（その画面を離れれば通常表示に戻る、表示だけの一時的な上書き）
...
| `MoreSavesPatches.cs` | 他mod「More Saves」が導入されている場合だけ、そのManual Saveごとに進捗データを個別保存・復元する（未導入でも動作に影響しない任意の連携） |
