...
## 新機能: More Saves連携（Manualセーブごとの進捗データ）

**背景**: このmodの進捗データ（`AreaProgressStore`）は`levelKey`（レベル単位の識別子）でしかキーを作っていない。ゲーム本体はMore Savesを使わない限りレベルごとに常に1つしかセーブを持たないため、通常はこれで矛盾無く機能する。しかしMore Savesを使って同じレベルに複数のManual Saveを作って使い分けると、それらは同じ`levelKey`を共有してしまい、どのセーブを読み込んでも「最後に書き込まれた1つの進捗データ」を共有してしまう（以前の調査で確認済み・見送り済みの既知の制限）。

**今回の対応方針（ユーザー確認済み）**: More Saves（Workshop ID `3239040787`, Zebra.MoreSaves）の`public`メソッド`SaveAllManual(string folderName)`/`LoadSave(string directory)`にHarmonyのpostfixを当て、Manual Save・その読み込みのタイミングを検知する。

- **Manual Save時**: `SaveManager`の`public`プロパティ`ManualDirectory`から、そのセーブのフォルダ（`ManualDirectory/folderName/`）を特定し、そこに**このmod独自の追加ファイル**（`F.AreaInfoDisplayOnPause.AreaProgress.xml`、メインの進捗ファイルと同名だが別の場所）として、現在の進捗データを無条件で書き出す
- **読み込み時**: `directory`引数が`ManualDirectory`配下（＝Manualセーブ）の場合だけ、同じ場所にこのmod独自のファイルが**あれば**読み込んで現在のメモリ上のデータを置き換える。無ければ何もしない（その回のManual Saveがこの機能が無い状態で作られた場合の自然なフォールバック）。Autoセーブの読み込みは何もしない（既存の単一ファイルで1:1のまま正しく機能しているため対応不要）

**実装上の工夫点**:
- `MoreSaves.dll`への直接の参照（csprojの`<Reference>`）は追加していない。`PlayTimeAccessor`/`EndingScreensAccessor`と同じ、`AccessTools.TypeByName`による完全リフレクション方式を採用（`MoreSavesPatches.cs`）。これによりMore Saves未導入環境でも型解決に失敗するだけで何も起きず、このmod自体の動作には一切影響しない
- `AreaProgressStore.Save(path)`はメインファイル用に`s_dirty`フラグが立っている時だけ書き込む設計だったが、スロットごとのスナップショットは「メインファイルが偶然dirtyかどうか」とは無関係に毎回書き込む必要がある。そのため`s_dirty`を無視して常に書き込む`SaveSnapshot(path)`を新設し、XML構築処理（`BuildXml`）を`Save`と共通化した
- スナップショットから復元した直後は、次回の通常の自動保存（`SaveLube.SaveCombinedSaveFile`のpostfix）でメインファイルにもこの内容が反映されるよう、`s_dirty`を強制的に立てる`LoadSnapshot(path)`を新設した（通常の`Load`はゲーム起動時の読み込み用で、こちらは`s_dirty`を立てない）

**満たすべき2つの制約（ユーザー指定、両方確認済み）**:
1. More Savesが無くてもこのmodは動作する → `Apply`の先頭で型が見つからなければ即`return`するため満たされる
2. このmod導入状態のManual Saveを、このmodを削除した後でも読み込める → More Saves自身の`LoadSave`は6つの既知ファイルだけを名前指定で読むため、こちらが追加したファイルの有無を一切気にしない。MoreSaves自身のファイル・実装は一切変更していないため満たされる

### 追加対応: Auto Saveも同じ仕組みでカバー

当初はManual Saveのみ対応していたが、「Auto側にも同じファイルを保存・復元できるようにするのは簡単か」という質問を受けて検討した結果、既存の仕組み（`AreaProgressStore.SaveSnapshot`/`LoadSnapshot`）をそのまま再利用できることを確認し、対応した。

- `SaveManager.SaveAllAuto()`（`public`・引数無し）にもpostfixを追加。`AutoDirectory`+`SaveName`（両方`public`プロパティ）からスロットのパスを計算する。本体の`SaveAllAuto`自身が`IsNameSet`（`SaveName`が空でないか）を見て何もしない場合があるのと同じ判定をこちら側でも真似て、`SaveName`が空の時は何もしない
